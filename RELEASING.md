...
- **日期** `YY.MMDD` = 你发版那天
- **`fork`** = 固定标识，一眼和上游的 `v26.0910.1` 区分开，也避免和上游 tag 撞名
- **序号** 当天从 1 开始，同一天再发就是 `.2`、`.3`，**第二天回到 1**

### 固定规则

| 规则 | 定义 |
|---|---|
| 打在哪 | `release` 分支当次发布的那个 commit，annotated tag |
| 何时产生 | 每跑一次 `publish_release.sh` 出一个新号 |
| 可变性 | tag 发出即**冻结**，不移动、不 force-push。发错了删掉重发下一个号 |
| 防重发 | 当前 commit 已经有 tag 时脚本直接报错拦下 |
| 上游基线 | **不进 tag**，只写进 Release 说明（`基于上游 9fa43e37（2026-09-24）`） |
